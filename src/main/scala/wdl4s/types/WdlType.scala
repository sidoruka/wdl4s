...
trait WdlType[U <: WdlType[U]] { this: U =>
   ...
   */
  protected def coercion: PartialFunction[Any, WdlValue[U]]

  /**
   * Public interface for a `Try`-wrapped conversion of an input of type `Any` to
   * a `WdlValue`.
   */
  def coerceRawValue(any: Any): Try[WdlValue[U]] = {
    any match {
      case v: WdlValue[U] => Success(v)
  ...
  def isCoerceableFrom[V <: WdlType](otherType: WdlType[V]): Boolean = false
...
object WdlType {
  ...
  val wdlTypeCoercionOrder: Seq[WdlType[_]] = Seq(
